...
const picotool_path =
  platformDirectory + "/tools/picotool/4.1.0-1aec55e/picotool.exe";
...
async function compile(rawCode, boardName, config, cb) {
  ...
  compiler.defines = `-DCFG_TUSB_MCU=OPT_MCU_pico ${build.usbvid} ${build.usbpid} ${build.usbpwr} -DARDUINO_VARIANT="${build.variant ||
    GB.board.board_info.name
  }" -DTARGET_pico -DPICO_FLASH_SIZE_BYTES=${build.flash_total}`;

  compiler.includes = `-iprefix"${platform_path}" "@${platform_path}/lib/${build.chip}/platform_inc.txt" "@${platform_path}/lib/core_inc.txt" "-I${platform_path}/include"`;
  ...
  compiler.ldflags = `${compiler.wrap} -Wl,--cref -Wl,--check-sections -Wl,--gc-sections -Wl,--unresolved-symbols=report-all -Wl,--warn-common ${undefined_symbols_flags}`;
  ...
  recipe.linking_prelink2_pattern = `"${compiler.path}${compiler.S.cmd}" ${compiler.c.elf.flags} ${compiler.c.elf.extra_flags} -c "${platform_path}/boot2/${build.chip}/${build.boot2}.S" "-I${platform_path}/pico-sdk/src/${build.chip}/hardware_regs/include" "-I${platform_path}/pico-sdk/src/common/pico_binary_info/include" -o "${build_dir}/boot2.o"`;

  // Combine gc-sections, archives, and objects
  recipe.c_combine_pattern = `"${compiler.path}${compiler.c.elf.cmd}" "-L${build_dir}" ${compiler.c.elf.flags} ${compiler.c.elf.extra_flags} ${compiler.ldflags} "-Wl,--script=${build_dir}/memmap_default.ld" "-Wl,-Map,${build_dir}/firmware.map" -o "{elf_file}" -Wl,--start-group {object_files} "{archive_file}" "${build_dir}/boot2.o" "${platform_path}/lib/${build.chip}/ota.o" ${compiler.libraries.ldflags} "${platform_path}/lib/${build.chip}/${build.libpico}" "${platform_path}/lib/${build.chip}/${build.libpicow}"  ${compiler.libbearssl} -lm -lc ${build.flags.libstdcpp} -lc -Wl,--end-group`;

  // Create output (UF2 file)
  recipe.objcopy_uf2_pattern = `"${picotool_path}" uf2 convert "{elf_file}" "{uf2_file}" --family pico-arm-s --abs-block`;

  const plugins_includes_dir = [];

  // Gen include dir
  let includes_dir = [];
  includes_dir.push(`${platformDirectory}/sdk/cores/${build.chip}`); // platform sdk
  ...
  sources_cores_dir.push(`${platformDirectory}/sdk/cores/${build.chip}`); // platform sdk
...
async function flash() {
  ...
  let cmd = `"${picotool_path}" load "${uf2_file}"`;
  log(`Flashing with picotool... ${getFileName(uf2_file)}`);
  await execPromise(ospath(cmd));

  // reset device after flashing
  let resetCmd = `"${picotool_path}" reset`;
  log(`Resetting device...`);
  return execPromise(ospath(resetCmd));
}
